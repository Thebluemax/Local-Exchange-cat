...
if ($_REQUEST["type"] == "Offer") $p->page_title = "Crear una oferta";
if ($_REQUEST["type"] == "Want") $p->page_title = "Crear una demanda";

include("classes/class.listing.php");
include("includes/inc.forms.php");

if($cUser->member_id == "ADMIN") {
	$p->DisplayPage("No puedes crear ofertas o demandas siendo el administrador/a. <p>Puedes crear miembros desde el  <a href=admin_menu.php>Menú de Administración</a>.");	
	exit;
}
...
if($_REQUEST["mode"] == "admin") {  // Administrator is creating listing for another member
	...
	if (isset($_REQUEST["member_id"])) {
		...
		$ids->LoadMemberGroup();
		$form->addElement("select", "member_id", "¿Para qué miembro?", $ids->MakeIDArray());
	}
...
$form->addElement('text', 'title', 'Títol', array('size' => 30, 'maxlength' => 60));
$form->addRule('title','Introdueix un títol','required');
$form->registerRule('verify_not_duplicate','function','verify_not_duplicate');
$form->addRule('title','Ja tens una oferta o demanda amb aquest títol','verify_not_duplicate');
$category_list = new cCategoryList();
$form->addElement('select', 'category', 'Categoria', $category_list->MakeCategoryArray());

if(USE_RATES)
	$form->addElement('text', 'rate', 'Puntuació', array('size' => 15, 'maxlength' => 30));
else
...
$form->addElement('advcheckbox', 'set_expire_date', 'Aquesta oferta o demanda ha d´expirar automàticament?');
$today = getdate();
$options = array('language'=> 'ca', 'format' => 'dFY', 'minYear' => $today['year'],'maxYear' => $today['year']+5, 'addEmptyOption'=>'Y', 'emptyOptionValue'=>'0');
$form->addElement('date','expire_date', 'Expira', $options);
...
$form->addRule('expire_date','Ha de ser una data futura','verify_future_date');
$form->registerRule('verify_valid_date','function','verify_valid_date');
$form->addRule('expire_date','La data no és vàlida','verify_valid_date');
$form->registerRule('verify_category','function','verify_category');
$form->addRule('category', 'Tria una categoria', 'verify_category');
...
function process_data ($values) {
	...
	if($created) {
		$list .= "Element creat. Crear <A HREF=listing_create.php?type=".$_REQUEST["type"]."&mode=".$_REQUEST["mode"]."&member_id=".$member->member_id.">un altre</A>?";	
	} else {
		$cErr->Error("Hi ha hagut un error gravant l'element. Intenta-ho més tard.");
	}
    $member->Limites(); 
   $p->DisplayPage($list);
}
