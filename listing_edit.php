...
if ($_REQUEST["type"] == "Want") $p->page_title = "Editar demanda: ".$title;
...
$form->addElement('date','expire_date', 'Expira el', $options);
...
$form->addElement('advcheckbox', 'set_reactivate_date', 'Aquesta oferta o demanda estarà temporalment inactiva?');
$form->addElement('date','reactivate_date', 'Es reactivarà el', $options);
$form->addRule('reactivate_date','Ha de ser una data futura','verify_future_date');
$form->addRule('reactivate_date','La data no és vàlida','verify_valid_date');
...
function process_data ($values) {
    ...
    if($created) {
        $list .= 'S´han gravat els canvis. Editar <A HREF="listing_to_edit.php?mode='. $_REQUEST['mode'] .'&member_id='. $_REQUEST["member_id"] .'&type='. $_REQUEST["type"] .'">otro</A>?';    
    } else {
