...
if($_REQUEST["mode"] == "admin") {  // Administrator is editing a member's account
	...
	$form->addElement("header", null, "Editar Asociado/a " . $member->person[0]->first_name . " " . $member->person[0]->last_name);

	$form->addElement("html", "<TR></TR>");
	$form->addElement("hidden","mode","admin");
	$form->addElement("hidden","member_id",$_REQUEST["member_id"]);
	$form->addElement("select", "status", "Aceptar como asociado/a", array("A"=>"Si", "X"=>"No"));
	...
	$update_text="¿Con qué frecuencia recibirá emails?";
...
$form->addElement("static", null, null, null);
$form->addElement('submit', 'btnSubmit', 'Editar asociado/a');
//
// Define form rules
//
...
$form->addRule('last_name', 'Introduce un apellido', 'required');
...
// $form->addRule('member_id','Este ID ya está siendo usado','verify_unique_member_id');
$form->registerRule('verify_good_member_id','function','verify_good_member_id');
$form->addRule('member_id','No se permiten caracteres especiales','verify_good_member_id');
$form->registerRule('verify_good_password','function','verify_good_password');
$form->addRule('password', 'La contraseña debe contener al menos un número', 'verify_good_password');
$form->registerRule('verify_no_apostraphes_or_backslashes','function','verify_no_apostraphes_or_backslashes');
$form->addRule("password", "Es mejor no utilizar apóstrofes o comillas en contraseñas", "verify_no_apostraphes_or_backslashes");
$form->registerRule('verify_role_allowed','function','verify_role_allowed');
$form->addRule('member_role','No puedes asignar un mayor nivel de acceso del que tienes','verify_role_allowed');
$form->addRule('join_date', 'La fecha no puede estar en el futuro', 'verify_not_future_date');
...
//	$member->person[0]->fax_ext = $phone->ext;	
	...
	if($member->SaveMember()) {
		$list .= "Cambios realizados. <b>Añade una foto y genera el carnet de socio</b> pulsando <A HREF=member_edit.php?type=".$_REQUEST["type"]."&mode=".$_REQUEST["mode"]."&member_id=".$member->member_id.">aquí</A>. ";
        if ($values['status']=="A")
        {
            if ($values ['email']=="")
             $list .= "Esta persona tendrá que ser informada de que se ha activado su cuenta ya que no ha facilitado un correo electrónico. "; 
            else
            {
...
$headers .= "MIME-Version: 1.0\r\n";
$headers .= "Content-Type: text/html; charset=UTF-8\r\n";
             $mailed=mail($to,ACTIVE_MEMBER_SUBJECT, $html,$headers);
            //if (PEAR::isError($mailed)) {
             if (!$mailed) {
            	// $list.=$mailed->getMessage();
              $list .= "Ha fallado el intento de enviar un email. Probablemente se deba a un problema técnico. Contacta con tu administrador en ". PHONE_ADMIN .". <I>El nuevo asociado/a debe ser informado del id ('". $values["member_id"]. "') y la contraseña ('". $values["password"] ."').</I>";
               $mailed=mail(EMAIL_ADMIN,NEW_MEMBER_SUBJECT, " a nou member se ha incrit: ".$values["member_id"]."-".$values["email"]." i no ha rebut mail amb las dades.",$headers);                                                                             
               } else {
                 $list .= "Se ha enviado un email a '". $values["email"] ."' informándole de que su cuenta ha sido activada.";    
                                                                                                                                                                                 
                  } 
            }                                                                                           
        }
        } else {
		$cErr->Error("Ha habido un error. Prueba más tarde o avisa a nuestro administrador/a.");
	}
   $p->DisplayPage($list);
}
//
