...
if ($_REQUEST['type'] == "Offer") $p->page_title = 'Esborrar oferta';
if ($_REQUEST['type'] == "Want") $p->page_title = 'Esborrar demanda';
...
if ($listings_exist) {
	...
	$form->addElement('submit', 'btnSubmit', 'Esborrar');
} else {
	if($_REQUEST["mode"] == "self")
		$text = "No tens ";
	else
		$text = $member->PrimaryName() . " no té ";
	
		if ($_REQUEST['type'] == "Offer") $message = $text ."actualment cap oferta";
		if ($_REQUEST['type'] == "Want") $message = $text ."actualment cap demanda";
...
function process_data ($values) {
	...
	if($deleted == 1) 
		$list .= "1 element esborrat.";
	elseif($deleted > 1)
		$list .= $deleted . " elements esborrats.";	
	else
		$cErr->Error("Hi va haver un error esborrant l'element. Has comprovat tots els elements?");
	$member->Limites(); 	
   $p->DisplayPage($list);
}
