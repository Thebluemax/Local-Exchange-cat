...
$p->page_title = "Introduir valoració";
...
$output = "<B>Per a què intercanvi?</B><BR>";
...
foreach($tradegrp->trade as $trade) {	
	...
	if(!$feedback_member) {	// Member hasn't left feedback yet, lets show a simple link
		...
		$show = true;
		$description .= "<BR><B>". $feedback_other->RatingText() ." Valoració: </B>". $feedback_other->comment;
		$link = "feedback_reply.php";
	}
...
if($i == 0)
	$output .= "No hi ha intercanvis a què puguis valorar. Ja has puntuat teus intercanvis recents.";
