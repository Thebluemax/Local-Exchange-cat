...
class cLogEntry {
	...
	function SaveLogEntry () {
		...
		$insert = $cDB->Query("INSERT INTO ". DATABASE_LOGGING ." (admin_id, category, action, ref_id, log_date, note) VALUES ('". $this->admin_id ."', '". $this->category ."', '". $this->action ."', '". $this->ref_id ."', now(), ". $cDB->EscTxt($this->note) .");");
...
class cLogStatistics {
	function MostRecentLog ($category, $action=null) {
		...
		$query = $cDB->Query("SELECT max(log_date) FROM ". DATABASE_LOGGING ." WHERE category='". $category ."'". $exclusions .";");
