...
class cMember
{
	...
	function Login($user, $pass, $from_cookie=false) {
		...
		if($row = mysql_fetch_array($query)) {
			...
			if($row = mysql_fetch_array($query)) {
				$cErr->Error("El teu compte ha estat bloquejat per massa intents fallits. Si us plau, contacta amb <A HREF=contact.php>l'administració</A> per desbloquejar.");
			} else {
				
			$query = $cDB->Query("SELECT NULL FROM ".DATABASE_USERS." WHERE status = 'X' and member_id='". $user ."';");
			if($row = mysql_fetch_array($query)) {
				$cErr->Error("Encara no t'ha donat d'alta. Tingues una mica de paciència.");
				} else {



				$cErr->Error("Contrasenya o número d'identitat incorrecte. Si us plau, proba de nou, o veu <A HREF=password_reset.php>aquí</A> per reiniciar la contrasenya.", ERROR_SEVERITY_INFO);}
			}
	...
	function ChangePassword($pass) { // TODO: Should use SaveMember and should reset $this->password
		...
		if($update) {
			...
			$cErr->Error("Hi ha hagut un error redirigint la contrasenya. Si us plau intenta-ho més tard.");
...
//					<INPUT TYPE=HIDDEN NAME=location VALUE='".$_SERVER["REQUEST_URI"]."'>
		$output = "<DIV STYLE='width=60%; padding: 5px;'><FORM ACTION=login.php METHOD=POST>
					...
					<INPUT TYPE=HIDDEN NAME=location VALUE='".$_SERVER["REQUEST_URI"]."'>
					<TABLE class=NoBorder><TR><TD ALIGN=LEFT>ID miembro:</TD><TD ALIGN=LEFT><INPUT TYPE=TEXT SIZE=12 NAME=user></TD></TR>
					<TR><TD ALIGN=LEFT>Contraseña:</TD><TD ALIGN=LEFT><INPUT TYPE=PASSWORD SIZE=12 NAME=pass></TD></TR></TABLE>
					<DIV align=LEFT><INPUT TYPE=SUBMIT VALUE='Login'></DIV>
					</FORM></DIV>
					<BR>
					Si encara no tens una compte,  <A HREF=contact.php>contacta'ns </A> per unir-te al Banc del Temps.
					<BR>";	
		return $output;
	}
	...
	//		$output = "<FONT SIZE=1><A HREF='".SERVER_PATH_URL."/member_logout.php'>Logout</A>&nbsp;&nbsp;&nbsp;";
                $output = "<FONT SIZE=3><A id='pading_a' HREF='".SERVER_PATH_URL."/member_logout.php'><B>Sortir</B></A>";
		} else {
	//		$output = "<FONT SIZE=1><A HREF='".SERVER_PATH_URL."/member_login.php'>Login</A>&nbsp;&nbsp;&nbsp;";
                $output = "<FONT SIZE=3><A id='pading_a' HREF='".SERVER_PATH_URL."/member_login.php'><B>Entrar</B></A>";		
		}
	...
	function MustBeLevel($level) {
		...
		{
			$page = "<DIV Class='AccessDenied'>Ho sento, no teniu permís per accedir<BR></DIV>";
			$p->DisplayPage($page);
			exit;
	...
	function LoadMember($member, $redirect=true) {
		...
		{
			if ($redirect) {
				$cErr->Error("S'ha produït un error en accedir al membre (".$member.").  Si us plau, prova'l més tard.");
				include("redirect.php");			
			}
			return false;
		}
	...
	function SaveMember() {
		...
		if(!$update)
			$cErr->Error("No es poden desar els canvis per '". $this->member_id ."'. Si us plau, prova'l més tard.");
	...
	//Agregamos un security level para que no todos vean todo , solo admin(saldos,)
	function DisplayMember ($security_level=0) {
		$output = "<STRONG>Soci/a:</STRONG> ". $this->PrimaryName() . " (". $this->MemberLink().")"."<BR>";
        if($this->member_role<1 && $security_level>0){
        $output .= "<STRONG>Saldo de ". $this->PrimaryName() . ": </STRONG>";
        if ($this->balance <= 0 && $security_level>0)
		...
		$output .= "<STRONG>Actividad:</STRONG> ";
		if ($stats->most_recent == ""){
			$output .= "Sense intercanvis encara<BR>";
		}
		else{		
			$output .= '<A HREF="trade_history.php?mode=other&member_id='. $this->member_id .'">'. $stats->total_trades ." intercanvis en total</A> ";
			if($security_level>0){
				$output .=",sumando ". $stats->total_units . " ". strtolower(UNITS) . ", l'últim el dia ". $stats->most_recent->ShortDate() ."<BR>";
			}
		}
		...
		if(isset($feedbackgrp->feedback)) {
			$output .= "<b>Valoració:</b> <A HREF=feedback_all.php?mode=other&member_id=". $this->member_id . ">" . $feedbackgrp->PercentPositive() . "% votos positius</A> (" . $feedbackgrp->TotalFeedback() . " en total, " . $feedbackgrp->num_negative ." negatius y " . $feedbackgrp->num_neutral . " neutrals)<BR>";		
		}

		$joined = new cDateTime($this->join_date);
		$output .= "<STRONG>Apuntat:</STRONG> ". $joined->ShortDate() ."<BR>";
		...
		if($this->person[0]->phone1_number != ""&&$security_level>0)
			$output .= "<STRONG>Telèfon:</STRONG> ". $this->person[0]->DisplayPhone("1") ."<BR>";
		if($this->person[0]->phone2_number != ""&&$security_level>0)
			$output .= "<STRONG>Segon telèfon:</STRONG> ". $this->person[0]->DisplayPhone("2") ."<BR>";						
		if($this->person[0]->fax_number != ""&&$security_level>0)
		...
		foreach($this->person as $person) {
			...
			if($person->directory_list == "Y") {
				$output .= "<STRONG>Membre associat:</STRONG> ". $person->first_name ." ". $person->last_name ."<BR>";
				if($person->email != "")
					$output .= "<STRONG>Email de ". $person->first_name .":</STRONG> ". "<A HREF=email.php?email_to=". $person->email ."&member_to=". $this->member_id .">". $person->email ."</A><BR>";				
				if($person->phone1_number != "")
					$output .= "<STRONG>Telèfon de ". $person->first_name .":</STRONG> ". $person->DisplayPhone("1") ."<BR>";
				if($person->phone2_number != "")
					$output .= "<STRONG>Segon telèfon de ". $person->first_name .":</STRONG> ". $person->DisplayPhone("2") ."<BR>";						
				if($person->fax_number != "")
				$output .= "<STRONG>Fax de ". $person->first_name .":</STRONG> ". $person->DisplayPhone("fax") ."<BR>";				
			}
		}		
	return $output;	
	}
	// creamos una funcion para el miembro mismo se liste sin restriccciones
		function DisplayMemberSelf ($security_level=0) {
		$output = "<STRONG>Soci/a:</STRONG> ". $this->PrimaryName() . " (". $this->MemberLink().")"."<BR>";
        if($this->member_role<1 && $security_level>0){
        $output .= "<STRONG>Saldo de ". $this->PrimaryName() . ": </STRONG>";
        if ($this->balance <= 0 && $security_level>0)
    ...
    {  
        ...
        $p->DisplayPage("Les teves demandes estan desactivades.");    
        exit;
        }
       ...
       elseif ($this->balance >= 7 and $this->balance <12 and $tipo=="Oferta"){     
                ...
                // Desactivación de el protocolo de demandas necesarias para una oferta.
                $demandas_necesarias=-1;// comementar esta instrucción para volver al protocolo
                ...
                if($demandas_actuales < $demandas_necesarias) {
                    if ($almenos == 1)       
                    $p->DisplayPage("Tus ofertas están desactivadas. <BR>Necesitas crear al menos <B>". $almenos ." demanda </B>para que tus ofertas sean visibles.</BR>");
                    else
                    $p->DisplayPage("Tus ofertas están desactivadas. <BR>Necesitas crear al menos <B>". $almenos ." demandas </B>para que tus ofertas sean visibles.</BR>"); 
                     exit;
                        }    
        }
        		// se rompe otro seguro el valor de horas era 12 , cambio a 100
           elseif ($this->balance >= 100 and $tipo=="Oferta"){     
                    $p->DisplayPage("Tus ofertas están desactivadas. <BR>Has acumulado más de 12 horas de saldo.<B> Tienes que demandar algún servicio</B> para que tus ofertas sean visibles.</BR>"); 
                     exit;
        }
    }
...
class cMemberGroup {
	...
	function EmailListingUpdates($interval) {
		...
		$email_text = "";
		if($offered_text != "No se han encontrado.")
			$email_text .= "<h2>Llistes de Ofertes</h2><br>". $offered_text ."<p><br>";
		if($wanted_text != "No s'han trobat.")
			$email_text .= "<h2>Llistes de Demandes</h2><br>". $wanted_text;
		if(!$email_text)
    ...
    'To' => $to,
    'Subject' => SITE_SHORT_TITLE .": Ofertes i demandes noves o actualitzades en ". $period);
...
class cBalancesTotal {
	...
	//	$query = $cDB->Query("SELECT sum(balance) from ". DATABASE_MEMBERS .";");
		$frase_query="SELECT sum(balance), COUNT(*) from ". DATABASE_MEMBERS .";";
		$query = $cDB->Query($frase_query); 
		...
		if($row = mysql_fetch_array($query)) {
			...
			/*
			*esta modificación hecha por pablo deberá ser seteada, por las caracteristicas
			*de cada una de las organizaciones , en pocas palabras controla que si todos empiezan con Xhoras,
			*la cantidad de horas circulantes dividida entre los usuarios sea siempre X.
			*
			*la instruccion original es así: if($this->balance == (($this->usuarios-1))*3) -menos1 por el admin
			/ @mxml13
			*/
		
			if($this->balance == (($this->usuarios))*10)                 // pablox if($row[0] == 0 aunque ahora no es así     CORREGIR LUEGO QUE SERA -1 PORQUE BORRAMOS AL ADMIN
				return true;
			else
				return false;
		} else { 
