...
//		$this->VerifyTradeMembers();
		if($this->FindTradeFeedback($this->trade_id, $this->member_author->member_id)) {
			$cErr->Error("No se puede crear una doble valoración.");
			return false;
		}
		
		$insert = $cDB->Query("INSERT INTO ". DATABASE_FEEDBACK ."(feedback_date, status, member_id_author, member_id_about, trade_id, rating, comment) VALUES (now(), '".$this->status ."', '". $this->member_author->member_id ."', '". $this->member_about->member_id ."', ". $this->trade_id .", '".$this->rating ."', ". $cDB->EscTxt($this->comment) .");");
	...
	function LoadFeedback ($feedback_id) {
		...
		//pilar
		//echo "<br> SELECT feedback_date, ".DATABASE_FEEDBACK.".status, member_id_author, member_id_about, ".DATABASE_FEEDBACK.".trade_id, rating, comment, member_id_from, category FROM ".DATABASE_FEEDBACK.",". DATABASE_TRADES ." WHERE ".DATABASE_FEEDBACK.".trade_id=". DATABASE_TRADES .".trade_id AND feedback_id=".$feedback_id."";		

		$query = $cDB->Query("SELECT feedback_date, ".DATABASE_FEEDBACK.".status, member_id_author, member_id_about, ".DATABASE_FEEDBACK.".trade_id, rating, comment, member_id_from, category FROM ".DATABASE_FEEDBACK.",". DATABASE_TRADES ." WHERE ".DATABASE_FEEDBACK.".trade_id=". DATABASE_TRADES .".trade_id AND feedback_id=".$feedback_id.";");
		...
		if($row = mysql_fetch_array($query)) {		
			...
			$cErr->Error("Ocurrió un error al acceder a la tabla de valoraciones.  Por favor, inténtelo más tarde.");
			include("redirect.php");
		}		
	}
	...
	function FindTradeFeedback ($trade_id, $member_id) {
		...
		$query = $cDB->Query("SELECT feedback_id FROM ". DATABASE_FEEDBACK ." WHERE trade_id=". $trade_id ." AND member_id_author='". $member_id ."';");
...
class cFeedbackGroup {
	...
	function DisplayFeedbackTable($member_viewing) {		
		$output = "<TABLE BORDER=0 CELLSPACING=0 CELLPADDING=3 WIDTH=\"100%\"><TR BGCOLOR=\"#d8dbea\"><TD><FONT SIZE=2><B>Tipus</B></FONT></TD><TD><FONT SIZE=2><B>Data</B></FONT></TD><TD><FONT SIZE=2><B>Context</B></FONT></TD><TD><FONT SIZE=2><B>De</B></FONT></TD><TD><FONT SIZE=2><B>Comentari</B></FONT></TD></TR>";
		...
		foreach($this->feedback as $feedback) {
			...
			if($feedback->rating != POSITIVE) {
				if ($member_viewing == $feedback->member_about->member_id)
					$output .= "<BR><A HREF=feedback_reply.php?feedback_id=". $feedback->feedback_id ."&mode=self&author=". $member_viewing ."&about=".$feedback->member_author->member_id .">Responder</A> "; 
				elseif ($member_viewing == $feedback->member_author->member_id)
					$output .= "<BR><A HREF=feedback_reply.php?feedback_id=". $feedback->feedback_id ."&mode=self&author=". $member_viewing ."&about=".$feedback->member_about->member_id .">Ampliar comentario</A> ";
			}
...
class cFeedbackRebuttal {
	...
	function SaveRebuttal () {
		...
		$insert = $cDB->Query("INSERT INTO ". DATABASE_REBUTTAL ."(rebuttal_date, member_id, feedback_id, comment) VALUES (now(), '".$this->member_author->member_id ."', '". $this->feedback_id ."', ". $cDB->EscTxt($this->comment) .");");

		if(mysql_affected_rows() == 1) {
			$this->rebuttal_id = mysql_insert_id();	
			$query = $cDB->Query("SELECT rebuttal_date from ". DATABASE_REBUTTAL ." WHERE rebuttal_id=". $this->rebuttal_id .";");
			$row = mysql_fetch_array($query);
	...
	function LoadRebuttal ($rebuttal_id) {
		...
		$query = $cDB->Query("SELECT rebuttal_date, feedback_id, member_id, comment FROM ".DATABASE_REBUTTAL." WHERE rebuttal_id=".$rebuttal_id.";");
		...
		if($row = mysql_fetch_array($query)) {		
			...
			$cErr->Error("Ocurrió un error al acceder a la tabla de réplicas.  Por favor, inténtelo más tarde.");
			include("redirect.php");
		}		
	}
}	
...
class cFeedbackRebuttalGroup {
	...
	function LoadRebuttalGroup($feedback_id) {
		...
		$this->feedback_id = $feedback_id;
		$query = $cDB->Query("SELECT rebuttal_id FROM ".DATABASE_REBUTTAL." WHERE feedback_id=".$feedback_id." ORDER by rebuttal_date;");		
	...
	function DisplayRebuttalGroup($member_about) {
		...
		foreach($this->rebuttals as $rebuttal) {
			if($member_about == $rebuttal->member_author->member_id)
				$output .= "<BR><B>Respuesta: </B>";
			else
				$output .= "<BR><B>Ampliación: </B>";
