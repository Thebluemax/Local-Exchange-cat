...
class cNews {
    ...
    function SaveNewNews () {
        ...
        if(mysql_affected_rows() == 1) {
            ...
            $cErr->Error("No se ha podido grabar el elemento.");
            return false;
        }        
    }
    ...
    function SaveNews () {
        ...
        $update = $cDB->Query("UPDATE ".DATABASE_NEWS." SET title=". $cDB->EscTxt($this->title) .", description=". $cDB->EscTxt($this->description) .", expire_date='". $this->expire_date->MySQLDate(). "', sequence=". $this->sequence ." WHERE news_id=". $this->news_id .";");    
...
//        $this->ExpireNews();
        ...
        $query = $cDB->Query("SELECT title, description, expire_date, sequence FROM ".DATABASE_NEWS." WHERE  news_id=". $news_id.";");
        ...
        if($row = mysql_fetch_array($query)) {        
            ...
            $cErr->Error("Ha ocurrido un error al acceder a la tabla de noticias.  Por favor, inténtelo más tarde.");
...
class cNewsGroup {
    ...
    function MakeNewsSeqArray($current_seq=null) { // TODO: OK, this is just ugly...
        ...
        $prior_title = "La primera de la lista";                // all each time.
        $lead_txt = "";
        $follow_txt = "";
