...
class cCategory
{
	...
	function SaveNewCategory() {
		...
		$insert = $cDB->Query("INSERT INTO ". DATABASE_CATEGORIES ."(parent_id, description) VALUES (". $cDB->EscTxt($this->parent) .", '". $this->description ."');");
	...
	function SaveCategory() {
		...
		$update = $cDB->Query("UPDATE ". DATABASE_CATEGORIES ." SET parent_id=". $cDB->EscTxt($this->parent) .", description='". $this->description ."' WHERE category_id=". $this->id .";");
	...
	function LoadCategory($id) {
		...
		$query = $cDB->Query("SELECT parent_id, description FROM ".DATABASE_CATEGORIES." WHERE category_id='".$id."';");
	...
	function DeleteCategory() {
		...
		$delete = $cDB->Query("DELETE FROM ".DATABASE_CATEGORIES." WHERE category_id=". $this->id);
...
class cCategoryList {
	...
	function LoadCategoryList($active_only=false, $type="%", $redirect=false) {	
		...
		if($active_only) {
			$query = $cDB->Query("SELECT DISTINCT ".DATABASE_CATEGORIES.".category_id, ".DATABASE_CATEGORIES.".description FROM ".DATABASE_CATEGORIES.", ".DATABASE_LISTINGS." WHERE ".DATABASE_LISTINGS.".category_code =".DATABASE_CATEGORIES.".category_id AND status='". ACTIVE ."' AND type LIKE '".$type."' ORDER BY ". DATABASE_CATEGORIES .".description;");
		...
		if($i == 0) {
			if ($redirect) {
				$cErr->Error("There was an error accessing a category record.  Please try again later.");
				include("redirect.php");			
			} else {
				return false;
			}
