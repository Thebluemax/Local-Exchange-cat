...
class cLoginHistory {  
	...
	function LoadLoginHistory ($member_id) {
		...
		$query = $cDB->Query("SELECT total_failed, consecutive_failures, last_failed_date, last_success_date FROM ".DATABASE_LOGINS." WHERE member_id='".$member_id."';");
	...
	function SaveLoginHistory () {
		...
		$update = $cDB->Query("UPDATE ".DATABASE_LOGINS." SET total_failed=". $this->total_failed .", consecutive_failures=". $this->consecutive_failures .", last_failed_date=". $cDB->EscTxt($this->last_failed_date) .", last_success_date=". $cDB->EscTxt($this->last_success_date) ." WHERE member_id='".$this->member_id ."';");	
	...
	function SaveNewLoginHistory () {
		...
		$insert = $cDB->Query("INSERT INTO ". DATABASE_LOGINS ." (member_id, total_failed, consecutive_failures, last_failed_date, last_success_date) VALUES ('". $this->member_id ."', ". $this->total_failed .", ". $this->consecutive_failures .", ". $cDB->EscTxt($this->last_failed_date) .", ". $cDB->EscTxt($this->last_success_date) .");");
